  ...
  const toggleSpeech = async () => {
    try {
      if (isSpeechEnabled) {
        ...
        // 开启语音 - 选择第一个可用的语音服务
        const availableService = speakApiList.find(
          (item) => item.name !== "关闭"
        );
        if (availableService) {
          await setSpeakApi(availableService.name);
          toast.success(`语音播放已开启 (${availableService.name})`);
        } else {
          ...
          <>
            <AlertDialog>
              ...
              <AlertDialogContent className="rounded-2xl border-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl">
                <AlertDialogHeader>
                  <AlertDialogTitle className="text-lg font-semibold bg-linear-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
                    确认更新记忆
                  </AlertDialogTitle>
                ...
                <AlertDialogFooter className="gap-3">
                  ...
                  <AlertDialogAction
                    ...
                    className="rounded-xl bg-linear-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700"
                  >
                    确定
                  </AlertDialogAction>
                </AlertDialogFooter>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent className="rounded-2xl border-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl">
                <AlertDialogHeader>
                  <AlertDialogTitle className="text-lg font-semibold bg-linear-to-r from-red-600 to-pink-600 bg-clip-text text-transparent">
                    确认清除对话
                  </AlertDialogTitle>
                  <AlertDialogDescription className="text-gray-600 dark:text-gray-300">
                    清空当前界面显示的聊天记录，让界面更清爽。所有消息仍会保留在数据库中，可随时在设置中导出完整历史记录。
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter className="gap-3">
                  <AlertDialogCancel className="rounded-xl">
                  ...
                  <AlertDialogAction
                    onClick={onClearChat}
                    className="rounded-xl bg-linear-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700"
                  >
                    确定
                  </AlertDialogAction>
                </AlertDialogFooter>
          ...
          <Popover>
            ...
            <PopoverContent className="rounded-xl bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-gray-200/60 dark:border-gray-700/60">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-linear-to-r from-blue-500 to-purple-600 rounded-full"></div>
